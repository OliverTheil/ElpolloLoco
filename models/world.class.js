class World {
     ...
     */
    flipBottlesImg() {
        const second = (bottles, sec) => bottles.filter((e, i) => i % sec === sec - 1);
    ...
    checkCollisions() {
        ...
        setInterval(() => {
            this.level.enemies.forEach( (enemy, indexEnemy) => {
                if(this.character.isColliding(enemy) && this.level.enemies[indexEnemy].energy > 0 && this.run == true) {
                    this.character.hit(); 
    ...
    checkEndboss() {
        setInterval(() => {
            if(this.character.x >= 5500 && this.run == true) {
                this.endbossHealthbar.loadImages(this.endbossHealthbar.HEALTHBAR_IMAGES);
            };
    ...
    enableEndbossHealthbar() {
        setInterval(() => {
                ...
                if(this.character.x < 5500 || this.endbossHealthbar.energy <= 0) {
